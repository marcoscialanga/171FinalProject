...
# "good", i.e., neither ruin did occured immediately nor the surplus blew up.
t <- 0
T_values <- c(0)
U <- 100
U_values <- c(U)
...
while (t < 300) {
  ...
  T_values <- c(T_values, tau + tail(T_values, n = 1))
  x <- rexp(n = 1, 1/20)
  U <- U + 1.5*tau-x
...
plot(T_values, U_values, ylab = "Surplus", xlab = "Time", 
     main = "Simulation of Cramér-Lundberg Model", 
     ylim = c(min(c(0,U_values))-10,max(U_values)+10))
abline(h = 0, col = "red")
